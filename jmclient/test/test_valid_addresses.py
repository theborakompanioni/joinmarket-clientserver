...
def test_valid_bip341_scriptpubkeys_addresses():
    btc.select_chain_params("bitcoin")
    with open(os.path.join(testdir,"bip341_wallet_test_vectors.json"), "r") as f:
    ...
    for x in json_data["scriptPubKey"]:
        ...
        assert str(CCoinAddress.from_scriptPubKey(btc.CScript(sPK))) == addr
        print("and it converts correctly from scriptPubKey: {}".format(btc.CScript(sPK)))
    btc.select_chain_params("bitcoin/regtest")
